...
class VideoHandler extends BaseHandler {
     ...
     */
    validate(file) {
        return FileValidator.validate(file, 'video', this.config);
    }
    ...
    async generatePreview(file) {
        this.preview = await PreviewGenerator.generate(file, 'video');
        return this.preview;
    }
    ...
    async upload(file, options) {
        ...
        const timeoutId = setTimeout(() => controller.abort(), UPLOAD_CONFIG.timeout.video);
        ...
        try {
            ...
            if (error.name === 'AbortError') {
                throw new Error('上传超时，请检查网络连接或尝试压缩视频');
            }
     ...
     */
    getMessageType() {
        return UPLOAD_CONFIG.messageTypes.VIDEO;
    }
